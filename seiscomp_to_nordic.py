...
# -*- coding: utf-8 -*-
"""
Created on Tue May  4 09:05:40 2021
...
for line in seiscomp_file_lines:# se itera a traves de las lineas del boletin con el fin de extraer informacion del evento
    ...
    if (line.find('MLv')!=-1) and (line.find('Preferred')==-1) and (magnitude=='Null'): # see lee la magnitud
        magnitude=line[14:18]
...
station_number=str(len(station_list))# numero de estaciones encontradas
...
residual_RMS=add_blank(residual_RMS, 4)+'L'

###############################################################################################################
# se crea la primera linea del s-file, el header

s_file_header=' '+year+'  '+month+' '+day+' '+hour+minute+' '+seconds+' '+latitude+' '+longitude+depth
s_file_header=s_file_header+'  '+agency+station_number+residual_RMS+agency
...
for station_pick in station_list_xml:
    ...
    quality_indicator='E'# revisar Apendice A del manual de SEISAN pag 541..
    ...
    station_pick_line=add_blank_suffix(station_pick_line,46)+azimuth[:6]+' '
